...
public class Spline implements Path {
  ...
  /**
   * distance between the two side of the robot wheel to wheel in meters
   */
  ...
  private void computeControlPoints() {
    ...
    for (int i = 0; i < degree; i++) {
      BezierCurve curve = new BezierCurve(numberOfSteps, width, knots[i], points1[i], points2[i],
          knots[i + 1]);
      Collections.addAll(pathPoints, curve.getPathPoints());
      Point[] leftCurve = offsetPoints(false, curve.getPathPoints(), knots[i], points1[i],
          points2[i],
          knots[i + 1]);
      Point[] rightCurve = offsetPoints(true, curve.getPathPoints(), knots[i], points1[i],
          points2[i],
          knots[i + 1]);
    ...
    this.pathPoints = pathPoints.toArray(new Point[0]);
    this.leftPoints = leftPoints.toArray(new Point[0]);
    this.rightPoints = rightPoints.toArray(new Point[0]);
  }

  private Point[] offsetPoints(boolean isRight, Point[] points, Point CP0, Point CP1, Point CP2,
      Point CP3) {
