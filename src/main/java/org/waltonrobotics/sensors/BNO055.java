package org.waltonrobotics.sensors;

import edu.wpi.first.wpilibj.I2C;
import edu.wpi.first.wpilibj.Timer;

/**
 * Library ported from Adafruit Ardunio library. See below for original license info. IMU Library
 * not included, routines return arrays of double. Code does not use the sensor API from Adafruit --
 * call getVector/getQuaternion to get data. Otherwise use as directed.
 *
 ...
 *
 * Designed specifically to work with the Adafruit BNO055 Breakout. Pick one up today in the
 * adafruit shop! ------ http://www.adafruit.com/products
 *
 ...
 *
 * Written by KTOWN for Adafruit Industries. MIT license, all text above must be included in any
 * redistribution
 */
public class BNO055 {
  ...
  private enum Register {
    ...
    /**
     * Create a scalar register which represents an arbitrary number of bytes (assumed
     * little-endian)
     *
     * @param address The address of the register
    ...
    /**
     * Create a scalar register which represents and arbitrary number of bytes (assumed
     * little-endian) which can be signed
     *
     * @param address The address of the register
     * @param bytes The number of bytes which store the value at the register
    ...
    private int read(I2C bno055, int component) throws BNO055Exception {
      ...
      if (!bno055.read(address + component * bytes, bytes, buf)) {
        throw new BNO055Exception(
            "Error whilst reading value from BNO055 for register " + toString());
      }
    ...
    private int[] readVector(I2C bno055) throws BNO055Exception {
      ...
      if (!bno055.read(address, bytes * components, buf)) {
        throw new BNO055Exception(
            "Error whilst reading vector from BNO055 for register " + toString());
      }
    ...
    private int processBuf(byte[] buf, int index) {
      if (buf.length < index + bytes) {
        throw new IndexOutOfBoundsException(
            "Index is out of bounds when reading from register " + toString());
      }
    ...
    private void write(I2C bno055, int component, int data) throws BNO055Exception {
      ...
      for (int i = 0; i < bytes; i++) {
        if (bno055.write(address + component * bytes + i, (data >> (i * 8)) & 0x0ff)) {
          throw new BNO055Exception(
              "Error whilst writing value to BNO055 for register " + toString());
        }
      }
    }
  ...
  public static class BNO055Exception extends Exception {
    ...
    public BNO055Exception(String message, Throwable cause, boolean enableSuppression,
  ...
  public final class Offsets {
    ...
    public Offsets(int[] accelOffset, int[] gyroOffset, int[] magOffset, int accelRadius,
        int magRadius) {
   ...
   *
   * System Status (see section 4.3.58) 0 = Idle 1 = System Error 2 = Initializing Peripherals 3 =
   * System Iniitalization 4 = Executing Self-Test 5 = Sensor fusion algorithm running 6 = System
   * running without fusion algorithms
   *
   * Self Test Results (see section ) 1 = test passed, 0 = test failed Bit 0 = Accelerometer self
   * test Bit 1 = Magnetometer self test Bit 2 = Gyroscope self test Bit 3 = MCU self test 0x0F =
   * all good!
   *
   * System Error (see section 4.3.59) 0 = No error 1 = Peripheral initialization error 2 = System
   * initialization error 3 = Self test result failed 4 = Register map value out of range 5 =
   * Register map address out of range 6 = Register map write error 7 = BNO low power mode not
   * available for selected operation mode 8 = Accelerometer power mode not available 9 = Fusion
